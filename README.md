...
```bash
# morning-weather-agent 프로젝트 루트에서 실행
chmod +x scripts/run_briefing.sh

# plist 내 경로를 현재 디렉토리 기준으로 치환 후 설치
sed "s|/Users/YOUR_USERNAME/morning-weather-agent|$(pwd)|g" \
  com.weather-briefing.plist \
...
| 동작     | 인수              | `C:\Users\...\morning-weather-agent\main.py`             |
| 동작     | 시작 위치         | `C:\Users\...\morning-weather-agent`                     |
...
현재 카카오톡 "나에게 보내기"는 단방향 전송만 가능합니다. **텔레그램 봇**으로 전환하면 브리핑 발송과 양방향 대화를 모두 **무료**로 구현할 수 있습니다.
...
main.py
  │
  ├─ config_loader       config.yaml 로드·검증
  ├─ memory_manager      개인화 메모리 로드 (선호·피드백 이력)
  │
  ├─ [위치별 반복]
  │    ├─ geocoder           주소 → 위경도
  │    ├─ grid_converter     위경도 → 기상청 격자 좌표
  ...
  │    └─ air_quality_fetcher   에어코리아 API
  │
  ├─ message_generator   Claude AI — 개인화 메모리 + 피드백 이력 반영 브리핑 생성
  └─ telegram_sender     텔레그램 봇 브리핑 전송

[사용자 메시지 수신 시]
telegram_bot
  ├─ 주소·선호 변경 → config.yaml / memory 업데이트
  └─ 피드백 수신 → memory_manager 기록 저장
```

|                    | 카카오 나에게 보내기 | 카카오 채널 친구톡 | 텔레그램 봇 |
| ------------------ | -------------------- | ------------------ | ----------- |
| 아침 브리핑 (push) | 무료                 | 유료               | **무료**    |
| 사용자 → 봇 메시지 | 불가                 | 무료               | **무료**    |
| 설정 변경 대화     | 불가                 | 가능               | **가능**    |

양방향 채널이 확보되면 아래 고도화 항목들이 모두 실현 가능해집니다.

- `src/kakao_sender.py` 옆에 `src/telegram_sender.py` 추가, config에 봇 토큰 / chat_id 입력

### 개인화 메모리 — RAG

텔레그램 봇으로 사용자 입력 채널이 생기면, 선호와 피드백을 대화 중에 수집해 저장할 수 있습니다.

- 개인 컨텍스트: 출퇴근 수단, 날씨 민감도, 선호 옷차림 스타일 등을 대화로 설정
- 과거 이력: "지난주 우산 권고 → 실제 맑음" 같은 피드백을 메시지로 수집해 누적
- 유사 과거 날씨 검색이 필요한 수준이 되면 벡터 DB(ChromaDB 등) 도입 검토

### 정확도 피드백 루프

전날 예측과 실제 기상 데이터를 자동 비교해 Claude에게 컨텍스트로 제공합니다.

- 예: "어제 최고기온 23도 예측 → 실제 19도, 우산 권고 → 실제 맑음"
- Claude가 이 패턴을 보고 보정된 브리핑 생성
