...
브리핑에 반드시 포함할 내용:
1. 전체적인 날씨 요약 — 최고/최저 기온을 반드시 숫자(°C)로 명시, 하늘 상태 포함
2. 우산 필요 여부 — 출근(7~9시) 또는 퇴근(18~20시) 강수확률을 반드시 숫자(%)로 명시
3. 마스크/외출 주의 여부 — PM2.5 수치와 등급을 반드시 숫자로 명시 (예: PM2.5 8㎍/㎥ 좋음), "나쁨" 이상이면 마스크 권고
4. 기온별 옷차림 추천 — 집의 아침 최저기온과 회사의 낮 최고기온을 반드시 숫자(°C)로 명시하며 추천
...
형식 규칙:
- 첫 줄은 반드시 "🌤️ {today} 오늘 날씨" 형식으로 시작 (날씨 상태에 따라 이모지는 바꿔도 됨)
- 카카오톡은 마크다운 미지원 — **, __, ##, - 등 마크다운 기호 절대 사용 금지
- 소제목은 이모지로만 구분 (예: ☂️ 우산:, 😷 마스크:, 👗 옷차림:)
